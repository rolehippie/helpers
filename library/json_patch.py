...
class PatchManager(object):
    ...
    def __init__(self, module):
        ...
        if self.json_doc == "" and self.create:
            if self.create_type == "object":
                ...
                self.module.fail_json(
                    msg="invalid option for 'create_type': %s" % self.create_type)
    ...
    def write(self):
        ...
        self.module.atomic_move(tmpfile,
                                to_native(os.path.realpath(to_bytes(
                                    self.outfile, errors='surrogate_or_strict')), errors='surrogate_or_strict'),
...
class JSONPatcher(object):
    ...
    def validate_operation(self, members):
        ...
        if members['op'] not in allowed_ops:
            raise ValueError(
                "'%s' is not a valid patch operation" % members['op'])
        ...
        if members['op'] == 'add':
            if 'value' not in members:
                raise ValueError(
                    "'%s' is an 'add' but does not have a 'value'" % repr(members))
            ...
            try:
                ...
                try:
                    ...
                    if idx == (len(elements) - 1):  # this helps us stay idempotent
                        return None
                    raise PathError(
                        "specified index '%s' was not found in JSON array" % path)
        return next_obj
        ...
        if "/" not in path:  # recursion termination
            ...
            if isinstance(obj, dict):
                try:
                    ...
                    raise PathError(
                        "could not find '%s' member in JSON object" % path)
                ...
                try:
                    ...
                    if int(path) > len(obj):  # violation of rfc 6902
                        ...
                        raise PathError(
                            "could not find index '%s' in JSON array" % path)
